...
- the grep from the start menu, for some reason, doesn't search inside hidden
files
- <C-.> as a keybind doesn't work inside Nvim
- autocompletion is froken in python file
- escape sequences are shown in man pages
...
to have as add-on but would not replace the earlier one)
...
new powertoys config new terminal config

cmdheight could be made zero (best would be have something like noice to have
cmdline at the center instead)

tabline should only show when having more than one buffer

git autocompletion doesn't work with alias `g` 
